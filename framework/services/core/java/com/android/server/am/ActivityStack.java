...
// LSP start
    static String LSP_prevPackage = "";
    private void sendLSP(String message) {
        Intent intent = new Intent("kr.ac.snu.lsprofiler.intent.action.ACTIVITYSTACK");
...
// LSP end
    final boolean resumeTopActivityInnerLocked(ActivityRecord prev, Bundle options) {
        ...
        if (mResumedActivity == next && next.state == ActivityState.RESUMED &&
            ...
            if (DEBUG_STACK) mStackSupervisor.validateTopActivitiesLocked();
                // LSP start
                if (!ActivityStack.LSP_prevPackage.equals(next.packageName)) {
                    //Slog.v("LSP", "LSP Resuming : " + next.packageName);
                    if (mService != null) {
                        ActivityStack.LSP_prevPackage = next.packageName;
                        sendLSP(next.packageName);
                    }
                }   
                // LSP end
            return false;
