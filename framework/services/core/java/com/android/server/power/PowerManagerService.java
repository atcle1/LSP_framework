...
public final class PowerManagerService extends SystemService
    ...
    private void userActivityInternal(long eventTime, int event, int flags, int uid) {
        ...
        /* LSP end */
        synchronized (mLock) {
            bUpdated = userActivityNoUpdateLocked(eventTime, event, flags, uid);
            if (bUpdated) {
                updatePowerStateLocked();
            }
        }
        ...
        if (bUpdated) {
            if (event == PowerManager.USER_ACTIVITY_EVENT_TOUCH) {
                Intent intent = new Intent("kr.ac.snu.lsprofiler.intent.action.USER_ACTIVITY")
                .putExtra("type", "userActivity")
                .putExtra("time", eventTime)
    ...
    private void setWakefulnessLocked(int wakefulness, int reason) {
        if (mWakefulness != wakefulness) {
            ...
            switch (wakefulness) {
                case WAKEFULNESS_ASLEEP:
                state = "WAKEFULNESS_ASLEEP";
                break;
                case WAKEFULNESS_AWAKE:
                state = "WAKEFULNESS_AWAKE";
                break;
                case WAKEFULNESS_DREAMING:
                state = "WAKEFULNESS_DREAMING";
                break;
                case WAKEFULNESS_DOZING:
                state = "WAKEFULNESS_DOZING";
                break;
            }
            Slog.d(TAG, "setWakefulnessLocked() " + state + " " + wakefulness + " " +reason);
            /* LSP end */
